...
- 기사 제목 입력 → Gemini가 웹에서 직접 검색 & 읽기 → 컨테이너선 관련만 한국어 요약
Streamlit Cloud 배포 버전 (Gemini + Google Search)
"""
...
<style>
    ...
    .article-card { background:#f8faff; border-left:5px solid #1a73e8; border-radius:8px; padding:1.2rem 1.5rem; margin-bottom:1.2rem; }
    .not-relevant-card { background:#fff8f8; border-left:5px solid #ccc; border-radius:8px; padding:1rem 1.5rem; margin-bottom:0.8rem; }
    .badge { display:inline-block; background:#e8f0fe; color:#1a73e8; border-radius:12px; padding:2px 10px; font-size:0.8rem; margin-right:5px; margin-bottom:5px; }
    .score-high { color:#2e7d32; font-weight:bold; }
    .score-mid  { color:#f57c00; font-weight:bold; }
    .score-low  { color:#c62828; font-weight:bold; }
    .korean-summary { background:#fff; border-radius:6px; padding:0.8rem 1rem; margin-top:0.7rem; font-size:0.97rem; line-height:1.7; color:#222; }
    .section-label { font-size:0.8rem; font-weight:600; color:#888; text-transform:uppercase; letter-spacing:1px; margin-bottom:0.5rem; }
</style>
...
def get_model():
    ...
    model = genai.GenerativeModel(
        ...
        tools="google_search_retrieval",  # Google 검색 도구 활성화
    )
    return model
...
2. Determine if the article is related to **container ships** or **boxships** (including: containership orders, deliveries, charter rates, TEU capacity, MSC/Maersk/COSCO/Evergreen/CMA CGM/ONE/HMM operations, newbuilds, scrapping, container ports, etc.)
3. If related → include it in results with a Korean summary.
...
Return ONLY valid JSON (no markdown, no explanation) in this exact format:
  ...
  {{
    ...
    "source_url": "URL of the article found",
    "is_relevant": true,
...
If an article is NOT relevant, set:
- "is_relevant": false
- "korean_summary": ""
- "not_relevant_reason": "Brief English reason why it's not container/boxship related"
 
ARTICLE TITLES TO RESEARCH:
{titles_text}
"""
...
# Gemini 분석 실행
# ────────────────────────────────────────────────────────────
def analyze_titles(model, titles: list[str]) -> list[dict]:
    ...
    try:
        ...
        # JSON 파싱 실패 시 raw 응답 표시
        st.warning("⚠️ JSON 파싱 오류가 발생했습니다. 아래는 AI 원본 응답입니다:")
...
# TXT 보고서
# ────────────────────────────────────────────────────────────
def make_txt_report(articles: list) -> str:
    relevant = [a for a in articles if a.get("is_relevant")]
    lines = [
        "=" * 60,
        "  컨테이너선 / 박스선 뉴스 요약 보고서",
        f"  생성일시: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        f"  입력 기사 수: {len(articles)}건 / 관련 기사 수: {len(relevant)}건",
        "=" * 60 + "\n",
    ]
    for art in relevant:
        lines += [
...
with st.sidebar:
    ...
    st.markdown("""
    1. 오른쪽에 **기사 제목을 한 줄에 하나씩** 입력
    2. **분석 시작** 버튼 클릭
    3. Gemini가 웹에서 기사를 직접 검색
    4. 컨테이너선 관련 기사만 한국어로 요약
    5. 결과 다운로드
    """)
    st.divider()
    st.markdown("### 🏷️ 탐지 기준")
...
example_titles = """MSC orders 10 ultra-large containerships at CSSC
Maersk reports Q3 earnings amid freight rate pressure
Evergreen takes delivery of 16,000 TEU vessel
Oil tanker market sees record highs in spot rates
CMA CGM expands boxship fleet with new charter deals"""
 
titles_input = st.text_area(
    label="기사 제목 목록",
    placeholder=example_titles,
    height=220,
